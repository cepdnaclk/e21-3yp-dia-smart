...
export const careTeamService = {
  ...
  async getIncomingRequests(): Promise<RelationshipRequestDto[]> {
    const response = await api.get("/relationship-requests/incoming");
    return response.data.data;
  },

  async acceptRequest(requestId: number): Promise<RelationshipRequestDto> {
    const response = await api.patch(`/relationship-requests/${requestId}/accept`);
    return response.data.data;
  },

  async rejectRequest(requestId: number): Promise<RelationshipRequestDto> {
    const response = await api.patch(`/relationship-requests/${requestId}/reject`);
    return response.data.data;
  },
